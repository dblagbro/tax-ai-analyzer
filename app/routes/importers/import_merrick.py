...
bp = Blueprint("import_merrick", __name__)


@bp.route(URL_PREFIX + "/api/import/merrick/credentials", methods=["POST"])
@login_required
def api_merrick_save_credentials():
    data = request.get_json() or {}
    username = data.get("username", "").strip()
    password = data.get("password", "").strip()
    if not username or not password:
        return jsonify({"error": "username and password required"}), 400
    db.set_setting("merrick_username", username)
    db.set_setting("merrick_password", password)
    return jsonify({"status": "saved", "message": "Credentials saved."})


@bp.route(URL_PREFIX + "/api/import/merrick/cookies", methods=["POST"])
@login_required
def api_merrick_save_cookies():
    data = request.get_json() or {}
    ...
    db.set_setting("merrick_cookies", json.dumps(cookies_list))
    return jsonify({"status": "saved", "message": f"{len(cookies_list)} cookies saved.",
                    "count": len(cookies_list)})


@bp.route(URL_PREFIX + "/api/import/merrick/cookies", methods=["DELETE"])
@login_required
def api_merrick_clear_cookies():
    db.set_setting("merrick_cookies", "")
    return jsonify({"status": "cleared"})


@bp.route(URL_PREFIX + "/api/import/merrick/status", methods=["GET"])
@login_required
def api_merrick_status():
    user = db.get_setting("merrick_username") or ""
    cookies_raw = db.get_setting("merrick_cookies") or ""
    cookies_count = 0
    if cookies_raw:
        try:
...
@bp.route(URL_PREFIX + "/api/import/merrick/mfa", methods=["POST"])
@login_required
def api_merrick_mfa():
    data = request.get_json() or {}
    ...
    from app.importers.merrick_importer import set_mfa_code
    set_mfa_code(int(job_id), code)
    return jsonify({"status": "ok"})


@bp.route(URL_PREFIX + "/api/import/merrick/start", methods=["POST"])
@login_required
def api_import_merrick_start():
    data = request.get_json() or {}
    ...
    username = db.get_setting("merrick_username")
    password = db.get_setting("merrick_password")
    if not username or not password:
        return jsonify({"error": "Merrick Bank credentials not configured."}), 400
    ...
    def _run(jid, uname, pw, yrs, eid, eslug, ckies):
        ...
        try:
            from app.importers.merrick_importer import run_import
            result = run_import(
            ...
            db.log_activity("import_complete",
                            f"Merrick Bank: {total} transactions for {yrs}")
    ...
    threading.Thread(
        ...
        daemon=True, name=f"merrick-{job_id}",
    ).start()
